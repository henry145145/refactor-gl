...
function truncate(timestamp, step) {
   ...
   */
  const lowerBound = (0, _moment.default)(timestamp).subtract(step);
...
function getStepSeconds(step, start) {
   ...
   */
  const startMillliseconds = parseInt((0, _moment.default)(start).format('x'), 10);
  const endMilliseconds = parseInt((0, _moment.default)(start).add(step).format('x'), 10);
  return endMilliseconds - startMillliseconds;
}

function getPlaySliderParams(timestamps, timeGrain) {
  const minTimestamp = (0, _moment.default)(Math.min(...timestamps));
  const maxTimestamp = (0, _moment.default)(Math.max(...timestamps));
  let step;
  let reference;
