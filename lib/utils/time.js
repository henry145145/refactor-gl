...
exports.getPlaySliderParams = getPlaySliderParams;
...
function truncate(timestamp, step) {
   ...
   */
  var lowerBound = (0, _moment.default)(timestamp).subtract(step);
  ...
  return (0, _moment.default)(dateParts);
}
...
function getStepSeconds(step, start) {
   ...
   */
  var startMillliseconds = parseInt((0, _moment.default)(start).format('x'), 10);
  var endMilliseconds = parseInt((0, _moment.default)(start).add(step).format('x'), 10);
  return endMilliseconds - startMillliseconds;
}

function getPlaySliderParams(timestamps, timeGrain) {
  var minTimestamp = (0, _moment.default)(Math.min.apply(Math, timestamps));
  var maxTimestamp = (0, _moment.default)(Math.max.apply(Math, timestamps));
  var step;
  var reference;
  ...
  if (timeGrain.includes('/')) {
    ...
    if (parts[0].endsWith('Z')) {
      // ISO string
      reference = (0, _moment.default)(parts[0]);
      step = _moment.default.duration(parts[1]);
    } else {
      reference = (0, _moment.default)(parts[1]);
      step = _moment.default.duration(parts[0]);
    }
  } else {
    step = _moment.default.duration(timeGrain);
