...
function getBreakPoints(_ref, features, accessor) {
  ...
  if (formDataBreakPoints === undefined || formDataBreakPoints.length === 0) {
    ...
    var _extent = (0, _d3Array.extent)(features, accessor),
        minValue = _extent[0],
        maxValue = _extent[1];
...
function getBreakPointColorScaler(_ref2, features, accessor) {
  ...
  var colorScheme = Array.isArray(linearColorScheme) ? new _color.SequentialScheme({
    colors: linearColorScheme,
    id: 'custom'
  }) : (0, _color.getSequentialSchemeRegistry)().get(linearColorScheme);
  var scaler;
  var maskPoint;
